...
# Function to create and send an event
def create_event(service, event_details, attendees):
    """Create and send a calendar event.

    Args:
        service: Authenticated Google Calendar service.
        event_details (dict): Dictionary containing event details like start, end, summary, and location.
        attendees (list): List of email addresses to invite.
...
# Streamlit interface
def main():
    st.title("Google Calendar Event Sender")

    # Event input form
    with st.form("event_form"):
        summary = st.text_input("Event Summary", "Team Meeting")
        location = st.text_input("Location", "")
        description = st.text_area("Description", "Discuss project updates.")
        start_time = st.text_input("Start Time (YYYY-MM-DDTHH:MM:SS)", "2024-01-01T10:00:00")
        end_time = st.text_input("End Time (YYYY-MM-DDTHH:MM:SS)", "2024-01-01T11:00:00")
        attendees = st.text_area("Attendees (comma-separated emails)", "example1@gmail.com, example2@gmail.com")
    ...
    if submit:
        try:
            ...
            event = create_event(service, event_details, attendees_list)
            st.success(f"Event created successfully: {event['htmlLink']}")
        except Exception as e:
