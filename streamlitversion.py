...
def fetch_holidays(country, year, api_key):
    ...
    params = {
        ...
        "country": country,
        "year": year,
        "public": True  # Fetch only public holidays
    }
    response = requests.get(url, params=params)
    if response.status_code == 200:
        return response.json().get("holidays", [])
    else:
        st.error(f"Error fetching holidays: {response.status_code}")
        return []

# Holiday API Section
if st.session_state.page == "Holiday API":
    st.title("Holiday Calendar")

    # API Key (replace with your actual API key)
    api_key = st.secrets["holiday_api_key"]

    # Country and Year Input
    country = st.selectbox("Select Country (ISO Code):", ["US", "GB", "DE", "FR", "IN"])
    ...
    if st.button("Fetch Holidays"):
        ...
        if holidays:
            ...
            holiday_df = holiday_df[["date", "name", "observed"]]
            st.write(f"Public Holidays in {country} for {year}:")
            st.dataframe(holiday_df)
        else:
            st.write(f"No holidays found for {country} in {year}.")
